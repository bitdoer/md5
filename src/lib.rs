pub mod lib {
    ...
    pub fn convert_and_pad(ascii: &str) -> Vec<u32> {
        ...
        match rem.len() {
            ...
            2 => output.push((rem[0] as u32) + (rem[1] as u32).rotate_left(8) + 0x00800000),
            3 => output.push(
                (rem[0] as u32)
                    + (rem[1] as u32).rotate_left(8)
                    + (rem[2] as u32).rotate_left(16)
                    + 0x80000000,
            ),
            _ => output.push(0x00000080),
        }

        let amount_of_padding = ((16 - (output.len() % 16) as isize) - 2).rem_euclid(16) as usize;
        output.resize(output.len() + amount_of_padding, 0);
    ...
    pub fn md5_hash(data: Vec<u32>) -> [u32; 4] {
        ...
        // output is LE
        [
            u32::from_le_bytes(a0.to_be_bytes()),
            u32::from_le_bytes(b0.to_be_bytes()),
            u32::from_le_bytes(c0.to_be_bytes()),
            u32::from_le_bytes(d0.to_be_bytes()),
        ]
    }
}
